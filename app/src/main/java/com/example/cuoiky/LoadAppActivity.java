...
public class LoadAppActivity extends AppCompatActivity {
    final String DATABASE_NAME = "QLSVCuoiKi.db";
//    private static int SPLASH_SCREEN = 3000;
                    ...
                    public void onClick(View v) {
                        if(edtk.getText().length() != 0 && edmk.getText().length() != 0){
                            if (checkusername(edtk.getText().toString())==false) {
                                ...
                                MainActivity.sqLiteDatabase.execSQL("INSERT INTO user values('"+edtk.getText().toString()+"','"+edmk.getText().toString()+"')");
                                dialog.cancel();
                            }
                            else {
    ...
    public Boolean checkusernamePassword(String username , String password) {
        Cursor cursor = MainActivity.sqLiteDatabase.rawQuery("select * from user where username = ? and password =?" , new String[] {username, password});
    ...
    public Boolean checkusername(String username) {
        Cursor cursor = MainActivity.sqLiteDatabase.rawQuery("select * from user where username = ?" , new String[] {username});
        if(cursor.getCount()>0){
            return true;
        }
        else {
